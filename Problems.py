...
class RealProblem(Problem):
    ...
    def getFitnesses(self, k=1):
        fitnesses=[]
        sols=[ ]
        gray_sols=[]
        if self.dim==1:
            all_values=cod.getAllValues(self.lower_bounds[0], self.upper_bounds[0], self.num_bit_code*k)
            for val in all_values:
                chr_real=[val]
                sols.append(chr_real)
                gray_sols.append(cod.convertFromFloatToBin(chr_real[0], self.lower_bounds[0], self.upper_bounds[0], self.num_bit_code))
                fitnesses.append(self.computeFitnessFromReal(chr_real))
        if self.dim == 2:
            all_values = []
            for i in range(self.dim):
                all_values.append(cod.getAllValues(self.lower_bounds[i], self.upper_bounds[i], self.num_bit_code*k))
            for val1 in all_values[0]:
                for val2 in all_values[1]:
                    chr_real = [val1,val2]
                    sols.append(chr_real)
    ...
    def getOptimalSol(self, k):
        sols, gray_sols, fitnesses = self.getFitnesses(k)
        index_opt=np.argmin(np.array(fitnesses))
        
        return sols[index_opt], fitnesses[index_opt]
